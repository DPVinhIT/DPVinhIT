# Hi, I'm Phước Vinh 👋

### Information Systems Student | Backend Developer

I'm an Information Systems student at **University of Science, VNU-HCM (HCMUS)** with a strong interest in **Backend Engineering, Distributed Systems, and Data Engineering**.

Currently, I'm focusing on **Java & Spring Boot**, building RESTful APIs and backend systems while improving my knowledge of databases, microservices, distributed systems, and data processing.

My current career path:
<h1 align="center">Hi, I'm Phước Vinh <img src="https://media.giphy.com/media/hvRJCLFzcasr14ia7z/giphy.gif" width="35px"></h1>

<p align="center">
  <a href="https://git.io/typing-svg"><img src="https://readme-typing-svg.demolab.com?font=Fira+Code&pause=1000&color=2496ED&center=true&vCenter=true&width=435&lines=Information+Systems+Student;Backend+Developer;Future+Data+Engineer" alt="Typing SVG" /></a>
</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=YOUR_GITHUB_USERNAME&label=Profile%20Views&color=0e75b6&style=flat" alt="Profile views" />
</p>

I'm an Information Systems student at **University of Science, VNU-HCM (HCMUS)** with a strong interest in **Backend Engineering, Distributed Systems, and Data Engineering**.

Currently, I'm focusing on **Java & Spring Boot**, building RESTful APIs and backend systems while improving my knowledge of databases, microservices, distributed systems, and data processing.

My current career path: **Backend Engineering ➔ Data Engineering**

---

## 🚀 About Me

- 🎓 Information Systems student at **HCMUS**
- 💻 Focused on **Backend Development**
- ☕ Learning and building with **Java & Spring Boot**
- 🏗️ Interested in **Microservices & Distributed Systems**
- 🗄️ Working with **MySQL & MongoDB**
- ☁️ Working with **Cloudinary** for cloud-based media storage
- 🐳 Working with **Docker & containerized applications**
- ⚡ Exploring **Apache Kafka** for event-driven systems
- 🔥 Learning **Apache Spark & Big Data**
- 🔐 Interested in **secure authentication and API design**
- 📚 Continuously improving my software engineering skills
- 🎯 Career goal: **Backend Engineer → Data Engineer**

📫 **How to reach me:**
<p align="left">
  <a href="mailto:your.email@example.com"><img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" /></a>
  <a href="https://linkedin.com/in/your-linkedin-profile"><img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" /></a>
</p>

---

## 🛠️ Tech Stack

### 💻 Programming & Backend
<p align="left">
  <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white"/>
  <img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white"/>
  <img src="https://img.shields.io/badge/REST_API-005571?style=for-the-badge"/>
</p>

### 🗄️ Databases
<p align="left">
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white"/>
  <img src="https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white"/>
</p>

### 🔐 Authentication & Security
<p align="left">
  <img src="https://img.shields.io/badge/Spring_Security-6DB33F?style=for-the-badge&logo=springsecurity&logoColor=white"/>
  <img src="https://img.shields.io/badge/Keycloak-4D4D4D?style=for-the-badge&logo=keycloak&logoColor=white"/>
  <img src="https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=jsonwebtokens&logoColor=white"/>
</p>

### ⚡ Distributed Systems & Data Engineering
<p align="left">
  <img src="https://img.shields.io/badge/Apache_Kafka-231F20?style=for-the-badge&logo=apachekafka&logoColor=white"/>
  <img src="https://img.shields.io/badge/Apache_Spark-E25A1C?style=for-the-badge&logo=apachespark&logoColor=white"/>
</p>

### ☁️ Cloud & DevOps
<p align="left">
  <img src="https://img.shields.io/badge/Cloudinary-3448C5?style=for-the-badge&logo=cloudinary&logoColor=white"/>
  <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white"/>
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white"/>
  <img src="https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black"/>
</p>

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=YOUR_GITHUB_USERNAME&show_icons=true&theme=radium&hide_border=true&include_all_commits=true&count_private=true" alt="GitHub Stats" />
</p>
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_GITHUB_USERNAME&layout=compact&theme=radium&hide_border=true" alt="Top Languages" />
</p>

---

## 📌 Featured Projects

### 🏠 Booking-Room

A room rental and booking management system designed to support the process of customer registration, room and bed selection, deposit, rental contracts, handover, and checkout.

**Key areas:**
- Customer registration & Rental registration
- Room & bed availability management
- Deposit & rental contract handling
- Role-based access control (RBAC)

**Technologies:**
`Node.js` · `REST API` · `MySQL` · `JWT` · `Docker` · `Cloudinary`

---

### 📚 Book Service

A backend service for managing books and booking-related operations following a layered architecture.

**Technologies:**
`Java 21` · `Spring Boot 3` · `MongoDB` · `REST API` · `Spring Security` · `Keycloak`

---

### ⚡ Event-Driven Data Pipeline

A streaming data pipeline for processing file metadata events using an event-driven architecture. The system transports events, streams incoming data, and stores the processed results.

**Architecture:**

```text
  [Parser Service]
         │
         ▼
     [Kafka]
         │
         ▼
[Spark Structured Streaming]
         │
         ▼
     [MongoDB]
**Backend Engineering → Data Engineering**

---

## 🚀 About Me

- 🎓 Information Systems student at **HCMUS**
- 💻 Focused on **Backend Development**
- ☕ Learning and building with **Java & Spring Boot**
- 🏗️ Interested in **Microservices & Distributed Systems**
- 🗄️ Working with **MySQL & MongoDB**
- ☁️ Working with **Cloudinary** for cloud-based media storage
- 🐳 Working with **Docker & containerized applications**
- ⚡ Exploring **Apache Kafka** for event-driven systems
- 🔥 Learning **Apache Spark & Big Data**
- 🔐 Interested in **secure authentication and API design**
- 📚 Continuously improving my software engineering skills
- 🎯 Career goal: **Backend Engineer → Data Engineer**

---

## 🛠️ Tech Stack

### 💻 Programming & Backend

<p align="left">
  <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white"/>
  <img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white"/>
  <img src="https://img.shields.io/badge/REST_API-005571?style=for-the-badge"/>
</p>

### 🗄️ Databases

<p align="left">
  <img src="https://img.shields.io/badge/MySQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white"/>
  <img src="https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white"/>
</p>

### 🔐 Authentication & Security

<p align="left">
  <img src="https://img.shields.io/badge/Spring_Security-6DB33F?style=for-the-badge&logo=springsecurity&logoColor=white"/>
  <img src="https://img.shields.io/badge/Keycloak-4D4D4D?style=for-the-badge&logo=keycloak&logoColor=white"/>
  <img src="https://img.shields.io/badge/JWT-000000?style=for-the-badge&logo=jsonwebtokens&logoColor=white"/>
</p>

### ⚡ Distributed Systems & Data Engineering

<p align="left">
  <img src="https://img.shields.io/badge/Apache_Kafka-231F20?style=for-the-badge&logo=apachekafka&logoColor=white"/>
  <img src="https://img.shields.io/badge/Apache_Spark-E25A1C?style=for-the-badge&logo=apachespark&logoColor=white"/>
</p>

### ☁️ Cloud & Storage

<p align="left">
  <img src="https://img.shields.io/badge/Cloudinary-3448C5?style=for-the-badge&logo=cloudinary&logoColor=white"/>
</p>

### 🐳 DevOps & Tools

<p align="left">
  <img src="https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white"/>
  <img src="https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white"/>
  <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
</p>
---

## 📌 Featured Projects

### 🏠 Booking-Room

A room rental and booking management system designed to support the process of customer registration, room and bed selection, deposit, rental contracts, handover, and checkout.

**Key areas:**

- Customer registration
- Rental registration
- Room & bed availability
- Deposit & rental contract
- Customer/member management
- Checkout process
- Role-based access control

**Technologies:**

`Node.js` · `REST API` · `MySQL` · `JWT` · `Docker` · `Cloudinary`

---

### 📚 Book Service

A backend service for managing books and booking-related operations.

The service follows a layered backend architecture and provides RESTful APIs for working with book data.

**Technologies:**

`Java 21` · `Spring Boot 3` · `MongoDB` · `REST API` · `Spring Security` · `Keycloak`

---

### ⚡ Event-Driven Data Pipeline

A streaming data pipeline for processing file metadata events using an event-driven architecture.

The system uses Kafka to transport events, Spark Structured Streaming to process incoming data, and MongoDB to store the processed results.

**Architecture:**

```text
Parser Service
      │
      ▼
    Kafka
      │
      ▼
Spark Structured Streaming
      │
      ▼
   MongoDB
